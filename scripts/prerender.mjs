...
- Работаем и с лабораторными бриллиантами (сертификат IGI), и с природными (сертификат GIA).
...
async function main() {
  ...
  // Каждая страница пишется дважды: как <путь>/index.html и как <путь>.html.
  //
  // Netlify отдаёт каталог по адресу со слэшем, а адрес без слэша разворачивает
  // редиректом 301. Сам по себе он безобиден, но каталог отдаётся через прокси
  // с основного домена, и этот 301 уезжает в браузер посетителя, который затем
  // просит новый адрес у того же прокси. В Safari такая цепочка складывалась в
  // петлю: «Load cannot follow more than 20 redirections».
  //
  // Файл рядом с папкой закрывает вопрос в корне: обе формы адреса отдают
  // содержимое сразу, редиректа не возникает ни у посетителя, ни у поисковика.
  ...
  // Правила маршрутизации. Пишутся здесь, а не лежат статикой, потому что
  // зависят от набора страниц.
  //
  // Netlify считает канонической одну форму адреса и разворачивает вторую
  // редиректом 301. На отдельном сайте это незаметно, но каталог отдаётся через
  // прокси с основного домена: редирект уезжает в браузер, тот просит новый
  // адрес у прокси, и если направление редиректа когда-нибудь поменяется —
  // браузер, запомнивший прежнее, начинает ходить по кругу. Именно это и
  // случилось: «Load cannot follow more than 20 redirections».
  //
  // Правило со статусом 200 на каждую форму адреса убирает редиректы совсем.
  // Ходить по кругу становится нечему, и поисковик получает страницу с первого
  // запроса, без лишнего шага.
  // Обе формы задаются явно: со слэшем и без. Встроенная канонизация адресов
  // Netlify (pretty URLs) отключена в netlify.toml — иначе она успевает
  // развернуть одну форму в другую редиректом раньше любых правил.
  const routes = pages.flatMap(p => [
    `${p.path}     ${p.path}.html    200`,
    `${p.path}/    ${p.path}.html    200`,
  ]).join('\n');
  writeFileSync(join(DIST, '_redirects'),
    `# Обе формы адреса отдают страницу напрямую — редиректов между ними нет.\n` +
    `${routes}\n\n` +
