...
eq('shape_select убран из ступеней воронки', steps.includes('shape_select'), false);
eq('ступени воронки', steps, ['session_start','catalog_view','product_open','booking_open','wa_click']);
eq('вовлечённость считается отдельно', total.engagement.config_change.sessions, 1);
eq('выбор огранки — вовлечённость (A, B, C за оба дня)', total.engagement.shape_select.sessions, 3);
